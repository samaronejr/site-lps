...
3. Em `Primeira oferta`, escolha o `Período`, informe a `Turma` e, opcionalmente, `Horários`,
   `Local` e a `Equipe docente`. A oferta precisa de uma equipe com responsável, e o
   `Período` precisa ter um calendário declarado igual ao `Calendário` da disciplina.
...
3. Em `Primeira oferta`, escolha o `Período`, informe a `Turma` e, opcionalmente, `Horários`,
   ...
   O formulário traz duas linhas de membro, para você e para o responsável quando são
   pessoas diferentes. O `Período` precisa ter um calendário declarado igual ao
...
3. Professor pode publicar a unidade com `Publicar unidade` ou abrir `Editar` para ajustá-la.
   Unidade é conteúdo interno: ela organiza os materiais e não vira página pública própria.
