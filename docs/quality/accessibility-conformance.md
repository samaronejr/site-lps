...
| D8 | Pagination links and facet checkboxes were smaller than the 24 px minimum target. | WCAG 2.5.8 | `wp-content/themes/lps-theme/assets/css/theme.css` | Pagination links, standalone paragraph links, and facet checkboxes carry their own target size. |
| D9 | Reduced motion did not apply: `:where()` neutralization lost to the component rules, so buttons still animated for 120 ms. | WCAG 2.3.3 | `wp-content/themes/lps-theme/assets/css/theme.css` | The reduced-motion block now neutralizes duration at the same specificity as the component rules, verified in Chromium with `prefers-reduced-motion: reduce`. |
...
| D12 | Citation downloads (BibTeX, CSL-JSON) were 22 px high, below the 24 px minimum target. | WCAG 2.5.8 | `DiscoverySurfaces::render_publication`, `wp-content/themes/lps-theme/assets/css/theme.css` | The download pair is a control cluster with its own target size. Found by the live keyboard journey, not by automation. |
