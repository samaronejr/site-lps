...
export async function runSeoCli(argv, mode) {
	...
	if (argv.includes("--capture")) {
		...
		const documents = await crawl(baseUrl, { extraPaths: list("--extra-paths"), timeoutMs: number("--timeout", 120000), concurrency: number("--concurrency", 3) });
		const keep = list("--only");
		const selected = keep.length === 0 ? documents : documents.filter((doc) => keep.includes(doc.path));
		const count = await writeSnapshot(snapshotDirectory, baseUrl.replace(/\/+$/, ""), selected);
		process.stdout.write(`${JSON.stringify({ lane: mode, action: "capture", snapshot: snapshotDirectory, documents: count })}\n`);
		return;
	}
	...
	const snapshot =
		baseUrl === ""
			...
			: {
					...
					documents: await crawl(baseUrl, { extraPaths: list("--extra-paths"), timeoutMs: number("--timeout", 120000), concurrency: number("--concurrency", 3) }),
				};
