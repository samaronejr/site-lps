...
<form class="lps-signin-card" id="signin" method="post" action="${en ? "/en/sign-in/" : "/entrar/"}">
<h2 class="lps-card-title" id="signin-form">${esc(t(copy.form.legend, locale))}</h2>
<p class="lps-field">
...
<a class="lps-meta" href="${en ? "/en/contact/" : "/contato/"}">${esc(t(copy.form.lost, locale))}</a>
</div>
<button class="lps-button lps-button-primary lps-signin-submit" type="submit">${esc(t(copy.form.submit, locale))}</button>
<div class="lps-signin-divider" role="separator"><span>${esc(en ? "or" : "ou")}</span></div>
<a class="lps-button lps-button-ghost lps-sso-google" href="${en ? "/en/sign-in/" : "/entrar/"}" aria-label="${esc(en ? "Sign in with Google Workspace — @lps.ufrj.br accounts that already exist here" : "Entrar com Google Workspace — contas @lps.ufrj.br que já existam aqui")}"><svg class="lps-sso-icon" viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.92c1.7-1.57 2.68-3.88 2.68-6.62z" fill="#4285F4"/><path d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.92-2.26c-.8.54-1.84.86-3.04.86-2.34 0-4.32-1.58-5.03-3.7H.96v2.33A9 9 0 0 0 9 18z" fill="#34A853"/><path d="M3.97 10.71a5.4 5.4 0 0 1 0-3.42V4.96H.96a9 9 0 0 0 0 8.08l3.01-2.33z" fill="#FBBC05"/><path d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.59C13.46.9 11.43 0 9 0A9 9 0 0 0 .96 4.96l3.01 2.33C4.68 5.16 6.66 3.58 9 3.58z" fill="#EA4335"/></svg><span>${esc(en ? "Sign in with Google" : "Entrar com Google")}</span></a>
