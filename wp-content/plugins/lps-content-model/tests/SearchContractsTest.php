...
/** Locale search, relevance, facet, and pagination contracts. */
final class SearchContractsTest extends \PHPUnit\Framework\TestCase {
	/**
	 * Provides normalization cases.
	 *
	...
	/**
	 * Verifies that normalizes accents case spacing and markup.
	 *
	...
	/**
	 * Verifies that query boundary reports short long and malformed values.
	 */
	...
	/**
	 * Verifies that weighting locale isolation facets and stable pagination.
	 */
	...
	/**
	 * Verifies that and across facets or within one facet.
	 */
	public function test_and_across_facets_or_within_one_facet(): void {
		$records = array(
			...
			self::record(
				...
				array(
					'status' => array( 'active' ),
					'area'   => array( 'software-engineering' ),
				)
			),
		);
		$result  = SearchPolicy::search_records(
			$records,
			'sinal',
			'pt-br',
			array(
	...
	/**
	 * Verifies that defines every approved listing facet and rejects unknown values.
	 */
	public function test_defines_every_approved_listing_facet_and_rejects_unknown_values(): void {
		...
		self::assertSame(
			array( 'area' => array( 'signal-processing' ) ),
			SearchPolicy::sanitize_facets(
				array(
	...
	/**
	 * Verifies that ten thousand record reference query stays under budget.
	 */
	...
	/**
	 * Builds a deterministic searchable record.
	 *
	...
	private static function record( int $id, string $locale, string $type, string $title, string $summary, string $body, array $facets, array $exact = array() ): array {
		return array(
			...
			'exact'        => $exact,
			'facets'       => $facets,
			'published_at' => sprintf( '2025-01-%02d 00:00:00', ( $id % 28 ) + 1 ),
		);
	}
}
