...
final class MediaRenderingTest extends TestCase {
	...
	public function test_video_iframe_and_document_render_with_accessible_labels(): void {
		$video = MediaPolicy::render_video(
			array( 'block' => 'video', 'media_id' => 52, 'locale' => 'pt-br', 'label' => 'Visita ao laboratorio', 'caption' => 'Demonstracao do detector', 'autoplay' => false, 'placement' => 'content', 'transcript_url' => '/uploads/transcript.html', 'captions_url' => '/uploads/lab-tour.vtt' ),
		...
		$iframe = MediaPolicy::render_iframe( array( 'label' => 'Local document preview', 'placement' => 'content' ), self::valid_pdf_asset() );
		self::assertStringContainsString( 'title="Local document preview"', $iframe );
		self::assertStringContainsString( 'loading="lazy"', $iframe );
		$document = MediaPolicy::render_document( array( 'block' => 'document', 'media_id' => 63, 'locale' => 'en', 'label' => 'Admissions notice', 'essential' => true, 'html_equivalent' => '<p>Applications close on 30 September.</p>', 'placement' => 'content' ), self::valid_pdf_asset() );
