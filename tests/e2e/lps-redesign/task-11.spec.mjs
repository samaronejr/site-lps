...
test.beforeAll(async ({ browser }) => {
  ...
  const user =
    process.env.LPS_PUBLISHER_USER ||
    PROJECT_USERS[test.info().project.name] ||
    "lps-t11-publisher";
...
test("the Portuguese homepage renders the specified composition from governed selections", async ({
  browser,
}) => {
  ...
  const sections = await page
    ...
    .evaluateAll((nodes) => nodes.map((node) => node.getAttribute("data-home-section")));
  ...
  await expect(mission.locator('[data-home-action="research"]')).toHaveAttribute(
    "href",
    "/pt-br/pesquisa/",
  );
  await expect(mission.locator('[data-home-action="teaching"]')).toHaveAttribute(
    "href",
    "/pt-br/ensino/",
  );
  ...
  await expect(research.locator('a[href*="qa-home-area-sinais"]')).toContainText(
  ...
  await expect(page.locator('[data-home-section="infrastructure"] a')).toContainText(
    "Infraestrutura",
  );
  ...
  await expect(page.locator('[data-home-section="teaching"] a')).toHaveAttribute(
    "href",
    "/pt-br/ensino/",
  );
  ...
  await expect(journeys.locator('[data-home-journey="opportunities"]')).toHaveAttribute(
    "href",
    /\/pt-br\/oportunidades\/$/,
  );
  await expect(journeys.locator('[data-home-journey="collaborate"]')).toHaveAttribute(
    "href",
    /\/pt-br\/colabore\/$/,
  );
  await expect(journeys.locator('[data-home-journey="infrastructure"]')).toHaveAttribute(
    "href",
...
test("the English homepage renders the same composition in English", async ({ browser }) => {
  ...
  await expect(page.locator('[data-home-section="research"]')).toContainText(
    "Signal processing (home fixture)",
  );
  await expect(page.locator('[data-home-section="latest"]')).toContainText(
    "Home news (QA fixture)",
  );
  await expect(page.locator('[data-home-section="latest"]')).toContainText(
    "Home seminar (QA fixture)",
  );
  ...
  await expect(page.locator('[data-home-section="contact"] a')).toHaveAttribute(
    "href",
    /\/en\/contact\/$/,
  );
...
test("a newly authored approved news item appears after CMS publication without deployment", async ({
  browser,
}) => {
    ...
    .poll(
      async () => {
        const probe = await context.request.get(
          "/wp-json/lps-qa/v1/homepage?probe=snapshot&locale=pt-br",
          {
            headers: { "X-WP-Nonce": nonce },
          },
        );
...
test("revoked image rights degrade to text and the text-only homepage stays coherent", async ({
  browser,
}) => {
  test.setTimeout(600_000);
