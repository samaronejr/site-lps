...
/**
 * Task 19 — the documented faculty workflow, executed for real.
 *
 * The Portuguese handbook `docs/handbook/guia-docente-painel.md` documents the
 * dashboard with exact labels. This spec walks those documented steps against
 * the running site: the scoped task list, a profile proposal, the news
 * review loop, unit and material creation, release, withdrawal and the
 * next-term copy, plus the delegate boundary. Every screenshot the handbook
 * embeds is captured here, so the images can never drift from the workflow
 * they claim to show.
 *
 * The policy and renderer contracts are covered by the PHPUnit and vitest
 * suites; this spec proves the documented clicks complete the real journeys.
 */
...
// `127.0.0.1`.
const BASE_URL = process.env.LPS_BASE_URL ?? "http://localhost:8902";
const SHOTS = "docs/handbook/images";

const RUN = `t19-${Date.now().toString(36)}`;
...
async function record(postType, id) {
  const response = await page.request.get(`/wp-json/wp/v2/${postType}/${id}`, {
...
/** Captures one handbook screenshot of the full rendered page. */
async function shot(target, name) {
  await target.screenshot({ path: `${SHOTS}/${name}`, fullPage: true });
}

test.describe("task-19: the documented faculty workflow", () => {
  test.beforeAll(async ({ browser }) => {
    ...
    await loginAs(page, "lps-t19-publisher", "lps-t19-publisher-pass", true);
  ...
  async function ensureEditor(browser) {
    ...
    await loginAs(editorPage, "lps-t19-editor", "lps-t19-editor-pass", true);
    state.editorNonce = await restNonce(editorPage);
  }

  test("the publisher builds the documented offering fixture", async () => {
    ...
    // The granted offering: the professor leads, a second person co-teaches.
    ...
    await grant("lps-t19-professor", state.offeringPtId, "professor");
    await grant("lps-t19-professor", 0, "professor", "news");
    await grant("lps-t19-delegate", state.offeringPtId, "delegate");
  });

  test("the documented task list matches the professor's scope", async () => {
    ...
    // The handbook's task table: exactly these three tasks for this account.
    ...
    await shot(professorPage, "painel-inicial.png");
  });

  test("the documented profile proposal reaches review and approval", async ({ browser }) => {
    ...
    await expect(professorPage.locator("body")).toContainText("Propor alterações de perfil");
    ...
    await shot(professorPage, "perfil-proposta.png");
  ...
  test("the documented news loop: submit, reject with note, resubmit, approve", async ({
    ...
    await shot(professorPage, "noticias.png");
    ...
    // The editor rejects with the required note, as the handbook documents.
    await editorPage.goto("/pt-br/painel/revisao/", { waitUntil: "domcontentloaded" });
    const reviewForm = editorPage
      .locator('form[data-dashboard-form="lps_dashboard_review"]')
      .filter({ has: editorPage.locator(`input[name="post_id"][value="${state.newsId}"]`) })
      .first();
    await reviewForm.locator('input[name="note"]').fill("Corrija o resumo antes de reenviar.");
    await clickAndWait(editorPage, reviewForm.locator('button[value="reject"]'));
    await expect(editorPage.locator("[data-dashboard-notice]")).toBeVisible({ timeout: 30_000 });

    // The professor sees the note on the returned draft and resubmits.
    await professorPage.goto("/pt-br/painel/noticias/", { waitUntil: "domcontentloaded" });
  ...
  test("the documented unit and material steps create drafts", async () => {
    test.setTimeout(180_000);
    ...
    await expect(professorPage.locator("body")).toContainText("Área da oferta");
  ...
  test("the documented release and withdrawal complete on the real record", async () => {
    test.setTimeout(180_000);
    ...
    // "Publicar agora" releases the download, then "Publicar material" makes
    // the record public, exactly as the handbook orders them.
    ...
    await shot(professorPage, "material-liberado.png");
    ...
    // "Retirar" withdraws the material: the state chip changes and the
    // guarded download stops resolving. The withdraw control is the
    // single-submit action form that replaces the release form once the
    // material is released.
    const withdrawButton = resourceRow.locator('button[data-action="withdraw-resource"]');
    await clickAndWait(professorPage, withdrawButton);
    await expect(professorPage.locator("[data-dashboard-notice]")).toBeVisible({ timeout: 30_000 });
    await expect(resourceRow).toContainText("Retirado");
    const withdrawn = await record("resources", state.resourceId);
    expect(withdrawn.meta?._lps_release_state ?? withdrawn._lps_release_state).toBe("withdrawn");
  });

  test("the documented next-term copy produces a draft", async () => {
    test.setTimeout(300_000);
    await professorPage.goto(`/pt-br/painel/ofertas/${state.offeringPtId}/`, {
      waitUntil: "domcontentloaded",
    });
    ...
    await expect(copyForm).toContainText("Revisei a equipe docente para o novo período");
    await shot(professorPage, "copia-proximo-periodo.png");
    ...
    await copyForm.locator('input[name="title"]').fill(`Turma ${RUN} T03`);
    await copyForm.locator('input[name="team_reviewed"]').check();
    ...
    const copiedId = rows.find((row) => row.title?.rendered?.includes(`Turma ${RUN} T03`))?.id ?? 0;
    expect(copiedId, "the copy must persist as a draft").toBeGreaterThan(0);
    const copied = await record("offerings", copiedId);
    expect(copied.status).toBe("draft");
  });

  test("the documented delegate boundary holds: no publish controls", async ({ browser }) => {
    test.setTimeout(120_000);
    delegateContext = await browser.newContext({ baseURL: BASE_URL });
    delegatePage = await delegateContext.newPage();
    await loginAs(delegatePage, "lps-t19-delegate", "lps-t19-delegate-pass", true);
